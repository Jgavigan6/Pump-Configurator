...
const generateBOM = (config) => {
  ...
  // Add shaft end cover
  const sec = series120Data.shaftEndCovers.find(sec => sec.code === config.secCode);
  if (sec) {
    bom.push({
  ...
  if (config.pumpType !== 'single') {
    gearSizes.push(...config.additionalGearSizes);
  }
  
  gearSizes.forEach(size => {
    const housing = series120Data.gearHousings.find(h => h.code === size);
    if (housing) {
      bom.push({
  ...
  const driveGearPartNumber = series120Data.driveGearSets[config.shaftStyle]?.[driveGearKey];
  if (driveGearPartNumber && driveGearPartNumber !== 'N/A') {
    bom.push({
      partNumber: driveGearPartNumber,
      quantity: 1,
      description: `Drive Gear Set - ${config.gearSize}" with ${series120Data.shaftStyles.find(s => s.code === config.shaftStyle)?.description}`
    });
  }
  ...
  if (config.pumpType !== 'single') {
    config.additionalGearSizes.forEach(size => {
      const idlerSet = series120Data.idlerGearSets.find(set => set.code === size);
      if (idlerSet) {
        bom.push({
  ...
  // Add PEC Cover
  bom.push({
    partNumber: series120Data.pecCover.partNumber,
    quantity: 1,
    description: `PEC Cover - ${series120Data.pecCover.description}`
  });

  // Add small parts kit
  bom.push({
    partNumber: `${config.type}120-${config.pumpType === 'single' ? '1' : '2'}`,
    quantity: 1,
